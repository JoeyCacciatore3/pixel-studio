...
export function distanceTransform(
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (x < width - 1 && y > 0) minDist = Math.min(minDist, distance[index - width + 1]! + Math.SQRT2)
  ...
  for (let y = height - 1; y >= 0; y--) {
    for (let x = width - 1; x >= 0; x--) {
      ...
      if (x < width - 1 && y < height - 1) minDist = Math.min(minDist, distance[index + width + 1]! + Math.SQRT2)
      if (x > 0 && y < height - 1) minDist = Math.min(minDist, distance[index + width - 1]! + Math.SQRT2)
