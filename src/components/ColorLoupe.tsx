...
export default function ColorLoupe({ visible, x, y, screenX, screenY, gridSize = 5, pixelSize = 8 }: ColorLoupeProps) {
  ...
  useEffect(() => {
    ...
    for (let py = 0; py < sampleHeight; py++) {
      for (let px = 0; px < sampleWidth; px++) {
        ...
        if (pixelX >= 0 && pixelY >= 0 && pixelX < gridSize * pixelSize && pixelY < gridSize * pixelSize) {
