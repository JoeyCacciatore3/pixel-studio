 ...
 */
async function getBestPracticesFromContext7(
  failures: TestFailure[]
): Promise<Array<{ topic: string; recommendation: string; source: string }>> {
  ...
  const { lookupPlaywrightBestPractices } =
    await import('../tests/e2e/helpers/mcp-context7-helpers');
 ...
 */
async function getPatternsFromMemory(
  failures: TestFailure[]
 ...
 */
async function createActionPlanWithSequentialThinking(
  failures: TestFailure[]
): Promise<Array<{ priority: string; action: string; reason: string }>> {
...
async function generateFixSuggestions(
  failures: TestFailure[]
): Promise<Array<{ test: string; suggestion: string; code?: string }>> {
  ...
  for (const failure of failures.slice(0, 20)) {
    ...
    if (!code) {
      const similarPatterns = await matcher.findSimilarPatterns(failure.errorMessage, 'tests/e2e');
