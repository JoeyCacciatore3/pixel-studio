 ...
 */
export async function storeTestPattern(pattern: Omit<TestPattern, 'usageCount' | 'successRate' | 'lastUsed'>): Promise<void> {
  try {
          ...
          for (const node of nodes) {
            try {
              if (mcpMemory.mcp_memory_open_nodes) {
                const nodeData = (await mcpMemory.mcp_memory_open_nodes({ names: [node.name] })) as { nodes?: Array<{ observations?: string[] }> }
                const observations = nodeData?.nodes?.[0]?.observations || []

                // Extract pattern data from observations
                const description = observations.find((obs: string) =>
                  obs && !obs.startsWith('Category:') && !obs.startsWith('Tags:') && !obs.startsWith('ID:')
                ) || ''
                const code = observations.find((obs: string) =>
                  obs && obs.includes('test(') || obs.includes('async')
                ) || ''
                const category = observations.find((obs: string) =>
                  obs?.startsWith('Category:')
                )?.replace('Category: ', '') || 'general'
                const tags = observations.find((obs: string) =>
                  obs?.startsWith('Tags:')
                )?.replace('Tags: ', '').split(', ') || []
                const id = observations.find((obs: string) =>
                  obs?.startsWith('ID:')
                )?.replace('ID: ', '') || node.name
